    ...
    if (PD == 1)
    {
        for(size_t i = 0; i < point_list.size(); i++)
        {
            for (size_t j = 0; j < point_list[i].neighbour_list_1N.size(); j++)
            {
                ...
                Eigen::Matrix3d Kab_1 = compute_Kab_1(xi_1, L, l, V_eff, C1);
    ...
    else if (PD == 2)
    {
        for(size_t i = 0; i < point_list.size(); i++)
        {
            for (size_t j = 0; j < point_list[i].neighbour_list_2N.size(); j++)
            {
                ...
                Eigen::VectorXd R = 2 * C1 * ((1/A) - (1/a)) * V_eff * calc;
                Ra_2.insert(Ra_2.end(), R.begin(), R.end());

                Eigen::Matrix3d Kab_2 = compute_Kab_2(xi_1, xi_2, Xi_1, Xi_2, A, a, V_eff, C1);
    ...
    else if (PD == 3)
    {
        for(size_t i = 0; i < point_list.size(); i++)
        {
            for (size_t j = 0; j < point_list[i].neighbour_list_3N.size(); j++)
            {
                ...
                Eigen::VectorXd R = 3 * C1 * ((1/V) - (1/v)) * V_eff * calc * calc1;
                Ra_3.insert(Ra_3.end(), R.begin(), R.end());

                Eigen::Matrix3d Kab_3 = compute_Kab_3(xi_1, xi_2, xi_3, Xi_1, Xi_2, Xi_3, V, v, V_eff, C1);
