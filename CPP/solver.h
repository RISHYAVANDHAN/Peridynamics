...
inline double compute_psi(double C1, double L, double l) {
    double s1 = l / L;
    return 0.5 * C1 * L * std::pow((s1 - 1), 2);
}
...
inline Eigen::Matrix3d compute_Kab_1(const Eigen::Vector3d& xi_1, double L, double l, double V_eff, double C1) {
    ...
    const double scalar_term = ((1.0 / L) - (1.0 / l));
...
inline Eigen::Matrix3d compute_Kab_2(const Eigen::Vector3d& xi_1, const Eigen::Vector3d& xi_2, const Eigen::Vector3d& Xi_1, const Eigen::Vector3d& Xi_2, double A, double a, double V_eff, double C1) {
    ...
    const double xi1_dot_xi2 = xi_1.dot(xi_2);
    const double xi2_dot_xi2 = xi_2.dot(xi_2);
    const double xi1_dot_xi1 = xi_1.dot(xi_1);

    Eigen::Vector3d term_vec1 = xi1_dot_xi2 * xi_2 - xi2_dot_xi2 * xi_1;
    Eigen::Vector3d term_vec2 = xi1_dot_xi2 * xi_1 - xi1_dot_xi1 * xi_2;

    const Eigen::Matrix3d term1 = (2.0 * C1 * V_eff / std::pow(a, 3)) * (term_vec1 * term_vec1.transpose());
    const double scalar_term = (1.0 / a) - (1.0 / A);
    const Eigen::Matrix3d term2 = (2.0 * C1 * V_eff * scalar_term) * (xi_2 * xi_2.transpose() - xi1_dot_xi2 * I);
    const Eigen::Matrix3d term3 = (2.0 * C1 * V_eff / std::pow(a, 3)) * (term_vec1 * term_vec2.transpose());
    const Eigen::Matrix3d term4 = (2.0 * C1 * V_eff * scalar_term) * (xi_2 * xi_1.transpose() + xi1_dot_xi2 * I - 2.0 * xi_1 * xi_2.transpose());

    Kab = term1 + term2 + term3 + term4;
    return Kab;
}

inline Eigen::Matrix3d compute_Kab_3(const Eigen::Vector3d& xi_1, const Eigen::Vector3d& xi_2, const Eigen::Vector3d& xi_3, const Eigen::Vector3d& Xi_1, const Eigen::Vector3d& Xi_2, const Eigen::Vector3d& Xi_3, double V, double v, const double V_eff, const double C1) {
    Eigen::Matrix3d Kab = Eigen::Matrix3d::Zero();
    ...
    Eigen::Matrix3d term1 = (3.0 * C1 * V_eff * scalar_term) * (xi_23 * xi_23.transpose());
    Eigen::Matrix3d epsilon_matrix;
    epsilon_matrix << 0, -xi_3[2], xi_3[1], xi_3[2], 0, -xi_3[0], -xi_3[1], xi_3[0], 0;
    Eigen::Matrix3d term2 = (3.0 * C1 * V_eff * diff_term) * ((xi_12.dot(xi_3)) * epsilon_matrix);
    Eigen::Matrix3d term3 = (3.0 * C1 * V_eff * scalar_term) * (xi_23 * xi_31.transpose());
    Eigen::Matrix3d epsilon_matrix_2;
    epsilon_matrix_2 << 0, -xi_2[2], xi_2[1], xi_2[2], 0, -xi_2[0], -xi_2[1], xi_2[0], 0;
    Eigen::Matrix3d term4 = -(3.0 * C1 * V_eff * diff_term) * ((xi_12.dot(xi_3)) * epsilon_matrix_2);
    Eigen::Matrix3d term5 = (3.0 * C1 * V_eff * scalar_term) * (xi_23 * xi_12.transpose());
...
inline void calculate_r(int PD, std::vector<Points>& point_list, double NN, double C1, double delta) {
    ...
    for (size_t i = 0; i < point_list.size(); i++) {
        ...
        point_list[i].Kab_1.setZero();
        point_list[i].Kab_2.setZero();
        point_list[i].Kab_3.setZero();
        ...
        for (size_t j = 0; j < point_list[i].neighbour_list_1N.size(); j++) {
            ...
            point_list[i].V_eff = Vh / static_cast<double>(point_list[i].n1);
        ...
        for (size_t j = 0; j < point_list[i].neighbour_list_2N.size(); j++) {
            ...
            point_list[i].psi += compute_psi(C1, A, a);
            point_list[i].V_eff = (Vh * Vh) / static_cast<double>(point_list[i].n1 + point_list[i].n2);
        ...
        for (size_t j = 0; j < point_list[i].neighbour_list_3N.size(); j++) {
            ...
            point_list[i].psi += compute_psi(C1, V, v);
            point_list[i].V_eff = (Vh * Vh * Vh) / static_cast<double>(point_list[i].n1 + point_list[i].n2 + point_list[i].n3);
